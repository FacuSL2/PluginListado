			...
			<?php if ( ! empty( $src[0] ) ) : ?>
				<div class="stm-single-image video-preview" data-id="big-image-<?php echo esc_attr( $video_preview ); ?>">
					<a class="light_gallery_iframe" data-iframe="true" data-src="<?php echo esc_url( $gallery_video ); ?>">
						<img src="<?php echo esc_url( $src[0] ); ?>" class="img-responsive" alt="<?php esc_html_e( 'Video preview', 'stm_vehicles_listing' ); ?>"/>
					</a>
				</div>
			<?php endif; ?>
		<?php endif; ?>
			...
			<?php foreach ( $gallery as $gallery_image ) : ?>
				...
				<?php if ( ! empty( $src[0] ) ) : ?>
					<div class="stm-single-image" data-id="big-image-<?php echo esc_attr( $gallery_image ); ?>">
						<a href="<?php echo esc_url( $full_src[0] ); ?>" class="stm_light_gallery" rel="stm-car-gallery">
							<img src="<?php echo esc_url( $src[0] ); ?>" alt="<?php echo esc_attr( get_the_title( get_the_ID() ) ) . ' ' . esc_html__( 'full', 'stm_vehicles_listing' ); ?>"/>
						</a>
					</div>
				<?php endif; ?>
			<?php endforeach; ?>
				...
				<?php foreach ( $gallery as $gallery_image ) : ?>
					...
					<?php if ( ! empty( $src[0] ) ) : ?>
						<div class="stm-single-image" id="big-image-<?php echo esc_attr( $gallery_image ); ?>">
							<img src="<?php echo esc_url( $src[0] ); ?>" alt="<?php echo esc_html( get_the_title( get_the_ID() ) ) . ' ' . esc_html__( 'full', 'stm_vehicles_listing' ); ?>"/>
						</div>
					<?php endif; ?>
				<?php endforeach; ?>
			<?php endif; ?>
