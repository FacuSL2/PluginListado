...
<div class="stm_add_car_form_1">
	...
	<div class="stm-form-1-end-unit clearfix">
		<?php if ( ! empty( $data ) ) : ?>
			<?php foreach ( $data as $data_key => $data_unit ) : ?>
				...
				<div class="stm-form-1-quarter">
					...
					if ( ! empty( $data_unit['numeric'] ) && $data_unit['numeric'] ) :
						...
						$plchldr = esc_html__( 'Enter', 'stm_vehicles_listing' ) . $data_unit['single_name'];
						if ( ! empty( $data_unit['number_field_affix'] ) ) {
							$plchldr .= '(' . $data_unit['number_field_affix'] . ')';
						}
						?>
						...
						<input
								...
								placeholder="<?php echo $plchldr; //phpcs:ignore ?>"
						/>
					<?php else : ?>
						<select name="stm_s_s_<?php echo esc_attr( $data_unit['slug'] ); ?>">
							...
							?>
							<option value=""><?php esc_html_e( 'Select', 'stm_vehicles_listing' ); ?><?php esc_html( $data_unit['single_name'] ); ?></option>
							<?php
							if ( ! empty( $terms ) ) :
			...
			?>
			<?php do_action( 'stm_listings_load_template', 'add_car/step_1_additional_fields', $vars ); ?>
