...
if ( ! $get_params ) {
	if ( $show_registered ) { ?>
		...
		<div class="stm-form-1-quarter stm_registration_date">
			<input type="text" name="stm_registered"
				class="form-control stm-years-datepicker
				<?php
				if ( ! empty( $data_value ) ) {
					echo esc_attr( ' stm_has_value' );
				}
				?>
				"
				placeholder="<?php esc_html_e( 'Enter date', 'stm_vehicles_listing' ); ?>" value="<?php echo esc_attr( $data_value ); ?>"/>
			<div class="stm-label">
	...
	if ( $show_vin ) {
		...
		<div class="stm-form-1-quarter stm_vin">
			<input type="text" name="stm_vin" class="form-control
			<?php
			if ( ! empty( $data_value ) ) {
				echo esc_attr( ' stm_has_value' );
			}
	...
	if ( $show_history ) {
		...
		<div class="stm-form-1-quarter stm_history">
			<input type="text" name="stm_history_label" class="form-control
			<?php
			if ( ! empty( $data_value ) ) {
				echo esc_attr( ' stm_has_value' );
			}
